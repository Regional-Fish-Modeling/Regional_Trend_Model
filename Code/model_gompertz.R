model{
  ...
  # prior on carrying capacity random by site
  alpha.K ~ dnorm(0, 0.001)
  sigma.K ~ dunif(0, 100)
  tau.K <- pow(sigma.K, -2)
  for(i in 1:nSites){
    log(K[i]) <- alpha.K + site.K[i]
    site.K[i] ~ dnorm(0, tau.K)
  }
  K.0 <- mean(K[ ])
  ...
  for(i in 1:nSites){
    ...
    for(t in 2:nYears){
      ...
      log(lambda[i,t-1]) <- log(N[i,t-1] * exp(nu[t-1] + r * (1 - log(N[i,t-1] + 1) / log(K[i] + 1))) + iota) + log(survey.length[i,t])
    }
  }
