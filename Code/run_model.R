...
if(model == "gompertz") {
  inits <- function() list(N = N.init,
                           ...
                           sigma.K = runif(1, 1, 2),
                           alpha.K = runif(1, log(20), log(75)), 
                           iota = runif(1, 0, 1),
                           p.mean = runif(1, 0.5, 0.8))
  parameters <- c("N", "K.0", "alpha.K", "sigma.K", "p.mean", "N.region", "b1.p", "r", "lambda.0", "sigma.nu", "iota") #, "p")
  mod <- "Code/model_gompertz.R"
}

if(model == "independent") {
  inits <- function() list(N = N.init,
                           alpha.0 = runif(1, 1, 3),
                           sigma.0 = runif(1, 0.5, 3),
                           sigma.nu = runif(1, 0.1, 2),
                           sigma.rho = runif(1, 0.5, 2),
                           p.mean = runif(1, 0.5, 0.8))
  
  parameters <- c("N", "alpha.0", "sigma.0", "p.mean", "N.region", "b1.p", "sigma.rho", "sigma.nu", "b") #, "p")
  mod <- "Code/model_independent.R"
}
...
saveRDS(out, paste0("Output/", model, "/mcmc_", model, ".Rds"))
save.image(paste0("Output/", model, "/", model, ".RData"))
}
...
if(model == "gompertz") {
  jagsUI::traceplot(out, parameters = c("K.0", "alpha.K", "sigma.K", "p.mean", "b1.p", "r", "lambda.0", "sigma.nu", "iota"))
}
if(model == "simple_gompertz") {
  jagsUI::traceplot(out, parameters = c("K", "p.mean", "b1.p", "r", "lambda.0", "sigma.nu", "iota"))
}
if(model == "independent") {
  jagsUI::traceplot(out, parameters = c("alpha.0", "sigma.0", "p.mean", "b1.p", "sigma.rho", "sigma.nu", "b"))
}
